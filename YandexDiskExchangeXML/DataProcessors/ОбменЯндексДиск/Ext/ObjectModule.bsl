...
// Обменивает заранее полученный код авторизации на токен. Требуется для работоспособности других методов.
//
...
// Получает список файлов по переданному пути на Яндекс.Диске.
// 
// Параметры:
//  Путь     - Строка, например "disk:/фото/"
//  Смещение - Число - служебный параметр, используется для рекурсивного 
//		использования метода в постраничном чтении списка. 
// 
Процедура ПолучитьСписокФайлов(Знач Путь = "/", Знач Смещение = 0) Экспорт
	...
	Если Ответ._embedded.total > Ответ._embedded.limit + Ответ._embedded.offset Тогда
		ПолучитьСписокФайлов(Путь, Мин(Смещение + Ответ._embedded.limit, Ответ._embedded.total)); 
	КонецЕсли;
