...
Процедура Скачать(Команда)
	...
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.Тип = "dir" Тогда
		Возврат;
	КонецЕсли;
	Адрес = СкачатьНаСервере(ТекущиеДанные.Путь);
	ПолучитьФайл(Адрес, ТекущиеДанные.Имя + ?(ТекущиеДанные.Тип = "dir", ".zip", ""), Истина);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьФайлНаДиск(Команда)
...
Процедура ОбработкаВыбораФайла(ИменаВыбранныхФайлов, ДополнительныеПараметры) Экспорт
	...
	ЗагрузитьФайлНаДискНаСервере(ВыбранныйФайл);
...
&НаСервере
Процедура ЗагрузитьФайлНаДискНаСервере(ВыбранныйФайл)
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбработкаОбъект.ЗагрузитьФайлНаДиск(ВыбранныйФайл.Путь, ВыбранныйФайл.АдресХранилища);
КонецПроцедуры
...
&НаСервере
Функция СкачатьНаСервере(Путь)
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	Возврат ОбработкаОбъект.ПолучитьДанныеФайла(Путь);
КонецФункции
